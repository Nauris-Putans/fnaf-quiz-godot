...
const LEVEL_H:int = 1152	## height of levels (viewport) - only used by Zelda transition
const LEVEL_W:int = 648	## width of levels (viewport) - only used by Zelda transition
...
func _on_content_finished_loading(incoming_scene) -> void:
	...
	get_tree().set_current_scene(incoming_scene)
...
#	This block is only used by the zelda transition, which is a special case that doesn't use the loading screen
	...
	#	# once the tweens are done, do some cleanup
		...
		# Remove the old scene (make sure we free the actual scene root under _load_scene_into)
	if _scene_to_unload != null and _scene_to_unload != get_tree().root:
		var scene_root := _scene_to_unload
		while scene_root != null and scene_root.get_parent() != _load_scene_into:
			scene_root = scene_root.get_parent()

		if scene_root != null and is_instance_valid(scene_root):
			scene_root.queue_free()
			await scene_root.tree_exited
